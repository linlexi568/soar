...
def create_pid_programs(k_p, k_d, k_w, k_s, k_i):
    # u_tx = -k_p * smooth(pos_err_y, s=k_s) + k_d * vel_y - k_w * ang_vel_x - k_i * ema(pos_err_y)
    ...
    ema_err_y = UnaryOpNode('ema', TerminalNode('pos_err_y'), params={'alpha': ConstantNode(0.2)})
    ...
    # u_ty = k_p * smooth(pos_err_x, s=k_s) - k_d * vel_x + k_w * ang_vel_y + k_i * ema(pos_err_x)
    ...
    ema_err_x = UnaryOpNode('ema', TerminalNode('pos_err_x'), params={'alpha': ConstantNode(0.2)})
...
def run_debug():
    ...
    print(f"Testing with params: k_p={k_p}, k_d={k_d}, k_w={k_w}, k_s={k_s}, k_i={k_i}")
    ...
    prog_tx, prog_ty, prog_tz, prog_fz = create_pid_programs(k_p, k_d, k_w, k_s, k_i)
