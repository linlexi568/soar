...
def evaluate_params(env, scg_calc, k_p, k_d, k_w, k_s, k_z_p, k_z_d, num_steps=240):
    ...
    for step in range(num_steps):
        ...
        # Calculate Roll/Pitch for debug if needed, but not used in controller directly
...
def main():
    print("初始化 Isaac Gym 环境 (Figure8 Trajectory)...")
    ...
    print("开始 Nonlinear DSL 参数搜索 (Figure8)...")
    ...
    for i in range(num_random):
        k_p = random.uniform(0.1, 2.0)
        k_d = random.uniform(0.1, 1.5)
        k_w = random.uniform(0.1, 1.0)
        k_s = random.uniform(0.1, 3.0)
        k_z_p = random.uniform(0.5, 1.5)
        k_z_d = random.uniform(0.2, 0.8)
    ...
    if best_params is None:
        best_params = (1.0, 0.5, 0.3, 1.0, 1.0, 0.5)
    ...
    for i in range(num_local):
        ...
        k_p = max(0.1, min(3.0, k_p))
        k_d = max(0.0, min(2.0, k_d))
        k_w = max(0.0, min(1.5, k_w))
        k_s = max(0.1, min(4.0, k_s))
        k_z_p = max(0.1, min(2.0, k_z_p))
        k_z_d = max(0.0, min(1.0, k_z_d))
