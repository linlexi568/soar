#!/usr/bin/env python3
"""
使用 Optuna 对 Square 轨迹的 Smooth 控制律进行贝叶斯调参。
目标：最小化 SCG Cost
控制律结构：
...
def objective(trial):
    ...
    k_s = trial.suggest_float('k_s', 0.05, 2.0)
    ...
    for step in range(num_steps):
        ...
        # 注意：scg_position 只返回 pos，没有 vel。
        # 如果我们不传 target_vel，SCGExactRewardCalculator 会假设 target_vel=0。
        # 对于 Square 轨迹，大部分时间 vel 确实是常数（直线），但在转角处不是。
        # 不过 tune_square_sign_bo.py 也没有传 target_vel，所以为了公平比较，这里也不传。
...
if __name__ == "__main__":
    ...
    print("开始 Square (Smooth) 轨迹贝叶斯调参...")
