...
注意：严格按 SCG 定义，仅返回 state+action 的二次代价，不包含额外惩罚。
"""
...
class SCGExactRewardCalculator:
    ...
    注意：此计算器不做任何额外 shaping，只返回纯 SCG 代价。
    """
    ...
    def compute_step(
        ...
        """计算单步 reward（精确 SCG 公式，不含 lag_penalty）
        
        Returns:
            reward: [N] 每个环境的即时 reward（负值，仅 state + action 代价）
        """
        N = pos.shape[0]
        ...
        # 真实 SCG reward（仅 state + action，不含 lag）
    ...
    def get_components(self) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            'total_cost': self._total_state_cost + self._total_action_cost,  # 不含 lag
            'steps': self._steps,
        }
    
    def get_mean_reward(self) -> torch.Tensor:
        """返回每步平均 reward（纯 SCG，不含 lag）"""
        if self._steps == 0:
