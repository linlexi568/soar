...
def create_nonlinear_programs(k_p, k_d, k_w, k_s):
...
print(f"验证 Square 轨迹非线性控制 (Cost < 100)")
print(f"Params: k_p={k_p}, k_d={k_d}, k_w={k_w}, k_s={k_s}")
...
prog_tx, prog_ty, prog_tz, prog_fz = create_nonlinear_programs(k_p, k_d, k_w, k_s)
...
for step in range(240):
    ...
    if step % 48 == 0:
        err_xy = np.sqrt((pos[0].item() - target[0])**2 + (pos[1].item() - target[1])**2)
        print(f't={t:.1f}s | tgt=[{target[0]:.2f},{target[1]:.2f}] | pos=[{pos[0].item():.2f},{pos[1].item():.2f},{pos[2].item():.2f}] | xy_err={err_xy:.3f}')

env.close()
components = scg_calc.get_components()
print(f'Final Cost: {components["total_cost"][0].item():.2f}')
