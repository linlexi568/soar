...
_ISAAC_GYM_PY = None
# Check both the local folder and the potential external folder
for _candidate in [Path('/home/linlexi/桌面/soar/isaacgym/python'), ROOT / 'isaacgym' / 'python']:
    if _candidate.exists():
        # Verify it has the compiled bindings (check for gym_38.so or just the folder)
        _bindings = _candidate / 'isaacgym' / '_bindings' / 'linux-x86_64'
        if _bindings.exists():
...
TRIALS = 50                 # 随机搜索次数(LQR参数少,200次足够)
