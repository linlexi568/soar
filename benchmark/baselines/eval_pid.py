#!/usr/bin/env python3
"""
Evaluate PID Controllers using saved parameters from results/pid/*.json
Only reproduction, NO tuning.

Usage:
    python baselines/eval_pid.py --task circle --episodes 3
    python baselines/eval_pid.py --task all
"""
...
class IsaacPIDController:
    """PID controller - outputs normalized actions matching env's 6-dim input format."""
...
def load_params(task: str) -> dict:
    path = RESULTS_DIR / f"pid_{task}.json"
    if not path.exists():
...
def evaluate_pid(task: str, duration: float, episodes: int, num_envs: int, device: str = 'cuda') -> Dict[str, Any]:
    print(f"\n{'='*60}")
    print(f"Evaluating PID on {task.upper()} trajectory")
    ...
    try:
        ...
        for ep in range(episodes):
            ...
            t = 0.0
            tgt_pos0, tgt_vel0 = make_targets(t)
            ...
            for s in range(steps):
                ...
                for i in range(num_envs):
                    ...
                    action4 = controller.compute(pos, vel, quat, omega_obs, tgt_pos, tgt_vel)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate PID (reproduction only)")
    ...
    all_results = {}
    for task in tasks:
        metrics = evaluate_pid(task, args.duration, args.episodes, args.num_envs, device)
        if metrics:
            all_results[task] = metrics

    print("\n" + "=" * 80)
    print("PID EVALUATION SUMMARY")
    print("=" * 80)
