...
class IsaacPDFFController:
    ...
    def __init__(self,
                 ...
                 mass: float = 0.027,
                 g: float = 9.81,
                 z_int_gain: float = 0.5,
                 z_int_clip: float = 0.5):
    ...
    def compute(self, pos, vel, quat, omega, target_pos, target_vel=None, target_acc=None):
        ...
        acc_des[2] += self.g
        # small integral on z to remove bias (optional)
...
def _double_integrator_lqr_gains(q_pos: float, q_vel: float, r: float):
    """Continuous-time LQR gains for a 1D double integrator.

    System: x=[p,v], xdot = [[0,1],[0,0]] x + [[0],[1]] u
    Cost: integral (q_pos*p^2 + q_vel*v^2 + r*u^2) dt
    Returns gains (k_pos, k_vel) such that u = k_pos*(p_ref-p) + k_vel*(v_ref-v).
    """
    q_pos = float(max(q_pos, 0.0))
    q_vel = float(max(q_vel, 0.0))
    r = float(max(r, 1e-12))

    k_pos = math.sqrt(q_pos / r) if q_pos > 0.0 else 0.0
    p12 = math.sqrt(q_pos * r) if q_pos > 0.0 else 0.0
    k_vel = math.sqrt((q_vel + 2.0 * p12) / r) if (q_vel > 0.0 or p12 > 0.0) else 0.0
    return k_pos, k_vel


class IsaacLQRDoubleIntegratorController:
    """Classic (model-based) LQR for small-angle translational double-integrator outer-loop.

    Computes constant gains from (Q,R) for a 1D double integrator, applied independently to X/Y/Z.
    This controller intentionally does NOT use trajectory acceleration feedforward.
    """
    ...
    def __init__(self,
        ...
        self.k_pos_xy, self.k_vel_xy = _double_integrator_lqr_gains(self.q_pos_xy, self.q_vel_xy, self.r_xy)
        self.k_pos_z, self.k_vel_z = _double_integrator_lqr_gains(self.q_pos_z, self.q_vel_z, self.r_z)
    ...
    def compute(self, pos, vel, quat, omega, target_pos, target_vel=None, target_acc=None):
        ...
        acc_des = np.array([
            self.k_pos_xy * pos_err[0] + self.k_vel_xy * vel_err[0],
            self.k_pos_xy * pos_err[1] + self.k_vel_xy * vel_err[1],
            self.k_pos_z * pos_err[2] + self.k_vel_z * vel_err[2],
        ], dtype=np.float32)

        acc_des[2] += self.g
        fz = self.mass * float(acc_des[2])
        fz = float(np.clip(fz, 0.0, self.thrust_clip * self.hover_thrust))

        roll_des = -float(acc_des[1]) / self.g * self.att_scale
        pitch_des = float(acc_des[0]) / self.g * self.att_scale
        rpy_des = np.array([roll_des, pitch_des, 0.0], dtype=np.float32)
...
def build_controller_eval(
    algo: str,
    pid_mode: str = 'cascade',
    lqr_mode: str = 'pure',
    num_envs: int = 1,
    *,
    pdff_mode: str | None = None,
):
    if algo == 'pid':
        ...
        def eval_pdff(params: Dict[str, float], task: str, duration: float, episodes: int):
            ctrl = IsaacPDFFController(**params)
            mode = pdff_mode if pdff_mode is not None else lqr_mode
            if mode == 'pure':
                ctrl.z_int_gain = 0.0
            return evaluate_params(ctrl, task, duration, episodes, num_envs=num_envs)
        return eval_pdff
        ...
        def eval_lqr(params: Dict[str, float], task: str, duration: float, episodes: int):
            ctrl = IsaacLQRDoubleIntegratorController(**params)
            return evaluate_params(ctrl, task, duration, episodes, num_envs=num_envs)
        return eval_lqr
    else:
        raise ValueError('algo must be pid, pdff, or lqr')
...
def main():
    ...
    ap.add_argument('--algo', type=str, default='both', choices=['pid','pdff','lqr','both'])
    ap.add_argument('--pid-mode', type=str, default='cascade', choices=['cascade','normal'])
    ap.add_argument('--pdff-mode', type=str, default='pure', choices=['pure','zi'])
    ap.add_argument('--episodes-per-eval', type=int, default=3)
    ap.add_argument('--trials', type=int, default=200)
    ap.add_argument('--output', type=str, default='results/baseline_isaac/{task}_pid_pdff_scg_exact.json')
    args = ap.parse_args()
    ...
    if args.algo in ('pdff','both'):
        ...
        bounds_pdff = {
            ...
            'k_yaw': (0.0, 8.0), 'k_yaw_rate': (0.0, 2.0),
            'att_scale': (0.1, 0.3),
        }
        eval_fn = build_controller_eval('pdff', pdff_mode=args.pdff_mode)
        best_pdff, met_pdff = local_random_search(base_pdff, bounds_pdff, args.trials, eval_fn,
                                                  args.task, args.duration, args.episodes_per_eval)
        results[f"best_pdff_{args.pdff_mode}"] = {'params': best_pdff, 'metrics': met_pdff}
        print(f"[PD-FF-{args.pdff_mode}] Best reward={met_pdff['mean_true_reward']:.3f}, rmse={met_pdff['rmse_pos']:.3f}")

    if args.algo == 'lqr':
        base_lqr = {
            'q_pos_xy': 1.0, 'q_vel_xy': 0.1, 'r_xy': 0.01,
            'q_pos_z': 1.0, 'q_vel_z': 0.1, 'r_z': 0.01,
            'k_att': 12.0, 'k_omega': 3.0,
            'k_yaw': 0.0, 'k_yaw_rate': 0.0,
            'att_scale': 0.2,
        }
        bounds_lqr = {
            'q_pos_xy': (0.0, 50.0), 'q_vel_xy': (0.0, 10.0), 'r_xy': (1e-4, 1.0),
            'q_pos_z': (0.0, 50.0), 'q_vel_z': (0.0, 10.0), 'r_z': (1e-4, 1.0),
            'k_att': (8.0, 25.0), 'k_omega': (2.0, 6.0),
            'k_yaw': (0.0, 8.0), 'k_yaw_rate': (0.0, 2.0),
            'att_scale': (0.1, 0.3),
        }
        eval_fn = build_controller_eval('lqr')
        best_lqr, met_lqr = local_random_search(base_lqr, bounds_lqr, args.trials, eval_fn,
                                                args.task, args.duration, args.episodes_per_eval)
        results["best_lqr_di"] = {'params': best_lqr, 'metrics': met_lqr}
        print(f"[LQR-DI] Best reward={met_lqr['mean_true_reward']:.3f}, rmse={met_lqr['rmse_pos']:.3f}")
